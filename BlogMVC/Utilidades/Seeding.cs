...
namespace BlogMVC.Utilidades
{
    public static class Seeding
    {
        ...
        public static void Aplicar(DbContext context, bool _)
        {
            ...
            {
                var rolDB = context.Set<IdentityRole>().FirstOrDefault(
                    x => x.Name == rol);

                if (rolDB is null)
                {
                    context.Set<IdentityRole>().Add(new IdentityRole { Name = rol,
        ...
        public static async Task AplicarAsync(DbContext context, bool _, CancellationToken cancellationToken)
        {
            foreach (var rol in roles)
            {
                var rolDB = await context.Set<IdentityRole>().FirstOrDefaultAsync(
                    x => x.Name == rol);
